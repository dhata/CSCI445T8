...
void moveCircle(Mat& img, char dir, double& angle, int& x, int&y, vector<Particle>& particles, char* main_window, vector<Point>& particleShoot, double sonarDistance){
	...
	if (dir == 'm'){ //for measure
		...
		circle(img, Point(expectedX,expectedY), 2, Scalar(255,255,0), -1, 8);
		return;
	}
	if (dir == 't'){
		updateAngle(angle);
